...
func _process(p_delta):
	if(p_delta > 0.0 and !Engine.is_editor_hint()):
		...
		process_label.set_text("Process: %s" % str(Performance.get_monitor(Performance.TIME_PROCESS)))
		physics_process_label.set_text("Physics Process: %s" % str(Performance.get_monitor(Performance.TIME_PHYSICS_PROCESS)))
		
		render_objects_in_frame_label.set_text("Render Objects in Frame: %s" % str(Performance.get_monitor(Performance.RENDER_OBJECTS_IN_FRAME)))
		render_draw_calls_in_frame_label.set_text("Render Draw Calls in Frame: %s" % str(Performance.get_monitor(Performance.RENDER_DRAW_CALLS_IN_FRAME)))
		render_verticies_in_frame_label.set_text("Verticies in Frame: %s" % str(Performance.get_monitor(Performance.RENDER_VERTICES_IN_FRAME)))
		render_material_changes_in_frame_label.set_text("Material Changes in Frame: %s" % str(Performance.get_monitor(Performance.RENDER_MATERIAL_CHANGES_IN_FRAME)))
		render_shader_changes_in_frame_label.set_text("Shader Changes in Frame: %s" % str(Performance.get_monitor(Performance.RENDER_MATERIAL_CHANGES_IN_FRAME)))
		render_surface_changes_in_frame_label.set_text("Surface Changes in Frame: %s" % str(Performance.get_monitor(Performance.RENDER_SURFACE_CHANGES_IN_FRAME)))
		
		memory_static_label.set_text("Memory Static: %s/%s kb" % [str(ceil(Performance.get_monitor(Performance.MEMORY_STATIC) * 0.001)), str(ceil(Performance.get_monitor(Performance.MEMORY_STATIC_MAX) * 0.001))])
		memory_dynamic_label.set_text("Memory Dynamic: %s/%s kb" % [str(ceil(Performance.get_monitor(Performance.MEMORY_DYNAMIC) * 0.001)), str(ceil(Performance.get_monitor(Performance.MEMORY_DYNAMIC_MAX) * 0.001))])
		
		object_count_label.set_text("Object Count: %s" % str(Performance.get_monitor(Performance.OBJECT_COUNT)))
		object_resource_count_label.set_text("Object Resource Count: %s" % str(Performance.get_monitor(Performance.OBJECT_RESOURCE_COUNT)))
		object_node_count_label.set_text("Object Node Count: %s" % str(Performance.get_monitor(Performance.OBJECT_NODE_COUNT)))
		
		physics_3d_active_objects_label.set_text("Physics 3D Active Objects: %s" % str(Performance.get_monitor(Performance.PHYSICS_3D_ACTIVE_OBJECTS)))
		physics_3d_collision_pairs_label.set_text("Physics 3D Collision Pairs: %s" % str(Performance.get_monitor(Performance.PHYSICS_3D_COLLISION_PAIRS)))
		physics_3d_island_count_label.set_text("Physics 3D Island Count: %s" % str(Performance.get_monitor(Performance.PHYSICS_3D_ISLAND_COUNT)))
		
		physics_2d_active_objects_label.set_text("Physics 2D Active Objects: " % str(Performance.get_monitor(Performance.PHYSICS_2D_ACTIVE_OBJECTS)))
		physics_2d_collision_pairs_label.set_text("Physics 2D Collision Pairs: " % str(Performance.get_monitor(Performance.PHYSICS_2D_COLLISION_PAIRS)))
		physics_2d_island_count_label.set_text("Physics 2D Island Count: " % str(Performance.get_monitor(Performance.PHYSICS_2D_ISLAND_COUNT)))
		
		usage_video_memory_total_label.set_text("Usage Video Memory Total: %s kb" % str(ceil(Performance.get_monitor(Performance.RENDER_USAGE_VIDEO_MEM_TOTAL) * 0.001)))
		video_memory_used_label.set_text("Video Memory Used Label: %s kb" % str(ceil(Performance.get_monitor(Performance.RENDER_VIDEO_MEM_USED) * 0.001)))
		texture_memory_used_label.set_text("Texture Memory Used: %s kb" % str(ceil(Performance.get_monitor(Performance.RENDER_TEXTURE_MEM_USED) * 0.001)))
		vertex_memory_used_label.set_text("Vertex Memory Used: %s kb" % str(ceil(Performance.get_monitor(Performance.RENDER_VERTEX_MEM_USED) * 0.001)))
...
func _ready():
	if(!Engine.is_editor_hint()):
		...
		render_material_changes_in_frame_label = get_node(render_material_changes_in_frame_label_path)
		...
		if(ProjectSettings.has_setting("debug/settings/performance/show_performance_monitor")):
			should_show_performance_monitor = ProjectSettings.get_setting("debug/settings/performance/show_performance_monitor")
		else:
