...
export (NodePath) var update_timer_path = NodePath()
...
var performance_label_update_rate: float = 0.5
var should_show_performance_monitor: bool = false
...
func _update_nodes() -> void:
	...
	update_timer = get_node(update_timer_path)
	assert(update_timer)

func _update_project_settings() -> void:
	if ProjectSettings.has_setting("debug/settings/performance/performance_label_update_rate"):
